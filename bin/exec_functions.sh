...
{	
	...
	#Query
	id_query=$(psql -c "SELECT ID FROM slk_query WHERE name = '$1'" ${DB_NAME} ${DB_USER} | sed '1,2d' | sed '/reg/d' | sed '/row/d' | sed '/^$/d' | sed '/./!d' | sed 's/ //g')
	...
	id_exec_time=$(psql -c "SELECT ID FROM slk_exec_time WHERE ID_QUERY = $id_query and TIME_INIT = '$time_init'" ${DB_NAME} ${DB_USER} | sed '1,2d' | sed '/reg/d' | sed '/row/d' | sed '/^$/d' | sed '/./!d' | sed  's/ //g')
	...
	#List of services
	psql -c "SELECT name FROM slk_service" ${DB_NAME} ${DB_USER} | sed '1,2d' | sed '/row/d' | sed '/reg/d' | sed '/^$/d' | sed '/./!d' > services.tmp
	...
	then
		...
		do
			...
			# id slave
			id_slave=$(psql -c "SELECT ID FROM slk_node WHERE name = '$slave'" ${DB_NAME} ${DB_USER} | sed '1,2d' | sed '/reg/d' | sed '/row/d' | sed '/^$/d' | sed '/./!d' | sed 's/ //g')
			
			for service in $(cat ./services.tmp)
			do
				# id service
				id_service=$(psql -c "SELECT ID FROM slk_service WHERE name = '$service'" ${DB_NAME} ${DB_USER} | sed '1,2d' | sed '/reg/d' | sed '/row/d' | sed '/^$/d' | sed '/./!d' | sed  's/ //g')
				...
				id_exec=$(psql -c "SELECT ID FROM slk_exec WHERE ID_EXEC_TIME = $id_exec_time and ID_SERVICE = $id_service and ID_NODE = $id_slave" ${DB_NAME} ${DB_USER} | sed '1,2d' | sed '/reg/d' | sed '/row/d' | sed '/^$/d' | sed '/./!d' | sed  's/ //g')
				...
				id_instr=$(psql -c "SELECT ID FROM slk_metric WHERE name = 'Instructions Retired'" ${DB_NAME} ${DB_USER} | sed '1,2d' | sed '/reg/d' | sed '/row/d' | sed '/^$/d' | sed '/./!d' | sed  's/ //g')
				id_clk_un=$(psql -c "SELECT ID FROM slk_metric WHERE name = 'Clock Uhalted'" ${DB_NAME} ${DB_USER} | sed '1,2d' | sed '/reg/d' | sed '/row/d' | sed '/^$/d' | sed '/./!d' | sed  's/ //g')
				id_clk_ref=$(psql -c "SELECT ID FROM slk_metric WHERE name = 'Clock Unhalted Ref'" ${DB_NAME} ${DB_USER} | sed '1,2d' | sed '/reg/d' | sed '/row/d' | sed '/^$/d' | sed '/./!d' | sed  's/ //g')
				id_temp=$(psql -c "SELECT ID FROM slk_metric WHERE name = 'Temperature'" ${DB_NAME} ${DB_USER} | sed '1,2d' | sed '/reg/d' | sed '/row/d' | sed '/^$/d' | sed '/./!d' | sed  's/ //g')
				id_runt=$(psql -c "SELECT ID FROM slk_metric WHERE name = 'Runtime'" ${DB_NAME} ${DB_USER} | sed '1,2d' | sed '/reg/d' | sed '/row/d' | sed '/^$/d' | sed '/./!d' | sed  's/ //g')
				id_r_un=$(psql -c "SELECT ID FROM slk_metric WHERE name = 'Runtime Unhalted'" ${DB_NAME} ${DB_USER} | sed '/reg/d' | sed '1,2d' | sed '/row/d' | sed '/^$/d' | sed '/./!d' | sed  's/ //g')
				id_cpi=$(psql -c "SELECT ID FROM slk_metric WHERE name = 'CPI'" ${DB_NAME} ${DB_USER} | sed '1,2d' | sed '/reg/d' | sed '/row/d' | sed '/^$/d' | sed '/./!d' | sed  's/ //g')
				id_e_pkg=$(psql -c "SELECT ID FROM slk_metric WHERE name = 'Energy PKG'" ${DB_NAME} ${DB_USER} | sed '1,2d' | sed '/reg/d' | sed '/row/d' | sed '/^$/d' | sed '/./!d' | sed  's/ //g')
				id_e_pp0=$(psql -c "SELECT ID FROM slk_metric WHERE name = 'Energy PP0'" ${DB_NAME} ${DB_USER} | sed '1,2d' | sed '/reg/d' | sed '/row/d' | sed '/^$/d' | sed '/./!d' | sed  's/ //g')
		...
		# id slave
		id_slave=$(psql -c "SELECT ID FROM slk_node WHERE name = '${HOSTNAME}'" ${DB_NAME} ${DB_USER} | sed '1,2d' | sed '/reg/d' | sed '/row/d' | sed '/^$/d' | sed '/./!d' | sed 's/ //g')
		...
		do
			...
			id_exec=$(psql -c "SELECT ID FROM slk_exec WHERE ID_EXEC_TIME = $id_exec_time and ID_SERVICE = $id_service and ID_NODE = $id_slave" ${DB_NAME} ${DB_USER} | sed '1,2d' | sed '/reg/d' | sed '/row/d' | sed '/^$/d' | sed '/./!d' | sed  's/ //g')
				...
				#Plot
			nline1=$(tac $PLOT_DIR/${HOSTNAME}.$service.gnu | grep -n -m 1 "plot" | cut -d ":" -f1)
			nline2=$(cat $PLOT_DIR/${HOSTNAME}.$service.gnu | wc -l)
			...
			done < <(tail -n "+$nline" $PLOT_DIR/${HOSTNAME}.$service.gnu)
		
			#Metrics

			instr=$(cat $PLOT_DIR/${HOSTNAME}.$service.dat | grep -i -m 2 "INSTR_RETIRED_ANY" | tail -1 | cut -d'|' -f4 | sed '/^$/d' | sed '/./!d' | sed  's/ //g') 
			clk_unhalted=$(cat $PLOT_DIR/${HOSTNAME}.$service.dat | grep -i -m 2 "CPU_CLK_UNHALTED_CORE" | tail -1 | cut -d'|' -f7 | sed '/^$/d' | sed '/./!d' | sed  's/ //g') 			
			clk_unhalted_ref=$(cat $PLOT_DIR/${HOSTNAME}.$service.dat | grep -i -m 2 "CPU_CLK_UNHALTED_REF" | tail -1 | cut -d'|' -f7 | sed '/^$/d' | sed '/./!d' | sed  's/ //g') 
			temp=$(cat $PLOT_DIR/${HOSTNAME}.$service.dat | grep -i -m 2 "TEMP_CORE" | tail -1 | cut -d'|' -f7 | sed '/^$/d' | sed '/./!d' | sed  's/ //g') 
			runtime=$(cat $PLOT_DIR/${HOSTNAME}.$service.dat | grep -i -m 2 "Runtime (RDTSC)" | tail -1 | cut -d'|' -f6 | sed '/^$/d' | sed '/./!d' | sed  's/ //g') 
			runtime_unhalted=$(cat $PLOT_DIR/${HOSTNAME}.$service.dat | grep -i -m 2 "Runtime unhalted" | tail -1 | cut -d'|' -f6 | sed '/^$/d' | sed '/./!d' | sed  's/ //g') 
			cpi=$(cat $PLOT_DIR/${HOSTNAME}.$service.dat | grep -i -m 2 "CPI" | tail -1 | cut -d'|' -f6 | sed '/^$/d' | sed '/./!d' | sed  's/ //g') 
			energy=$(cat $PLOT_DIR/${HOSTNAME}.$service.dat | grep -i -m 5 "Power" | tail -1 | cut -d'|' -f3 | sed '/^$/d' | sed '/./!d' | sed  's/ //g') 
			energy_pp0=$(cat $PLOT_DIR/${HOSTNAME}.$service.dat | grep -i -m 2 "Power PP0" | tail -1 | cut -d'|' -f3 | sed '/^$/d' | sed '/./!d' | sed  's/ //g') 
			...
			id_temp=$(psql -c "SELECT ID FROM slk_metric WHERE name = 'Temperature'" ${DB_NAME} ${DB_USER} | sed '1,2d' | sed '/reg/d' | sed '/row/d' | sed '/^$/d' | sed '/./!d' | sed  's/ //g')
			id_runt=$(psql -c "SELECT ID FROM slk_metric WHERE name = 'Runtime'" ${DB_NAME} ${DB_USER} | sed '1,2d' | sed '/reg/d' | sed '/row/d' | sed '/^$/d' | sed '/./!d' | sed  's/ //g')
			id_r_un=$(psql -c "SELECT ID FROM slk_metric WHERE name = 'Runtime Unhalted'" ${DB_NAME} ${DB_USER} | sed '1,2d' | sed '/reg/d' | sed '/row/d' | sed '/^$/d' | sed '/./!d' | sed  's/ //g')
			id_cpi=$(psql -c "SELECT ID FROM slk_metric WHERE name = 'CPI'" ${DB_NAME} ${DB_USER} | sed '1,2d' | sed '/reg/d' | sed '/row/d' | sed '/^$/d' | sed '/./!d' | sed  's/ //g')
			id_e_pkg=$(psql -c "SELECT ID FROM slk_metric WHERE name = 'Energy PKG'" ${DB_NAME} ${DB_USER} | sed '1,2d' | sed '/reg/d' | sed '/row/d' | sed '/^$/d' | sed '/./!d' | sed  's/ //g')
			id_e_pp0=$(psql -c "SELECT ID FROM slk_metric WHERE name = 'Energy PP0'" ${DB_NAME} ${DB_USER} | sed '1,2d' | sed '/reg/d' | sed '/row/d' | sed '/^$/d' | sed '/./!d' | sed  's/ //g')
