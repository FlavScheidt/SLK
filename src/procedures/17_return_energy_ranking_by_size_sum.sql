CREATE OR REPLACE FUNCTION return_energy_ranking_by_size_sum ( p_sz BIGINT ) 
	RETURNS TABLE ( query VARCHAR(50), job BIGINT, sign INT, sm DOUBLE PRECISION, peak DOUBLE PRECISION ) as $$	
DECLARE 
	q VARCHAR(50);
BEGIN
	...
	CREATE TEMPORARY TABLE temp_slk_plots AS
		SELECT 
			...
			temp_slk_exec_time.sign				as sign,
	...
	CREATE TABLE temp_slk_plots_sum AS
		...
		GROUP BY temp_slk_plots.id_exec_time, temp_slk_plots.query, temp_slk_plots.job, temp_slk_plots.sign
		ORDER BY sm DESC
	;
	...
	RETURN QUERY
		SELECT DISTINCT
			...
			temp_slk_plots_sum.sign					as sign,
			AVG(temp_slk_plots_sum.sm)				as sm,
			AVG(temp_slk_plots_sum.peak)			as peak
		FROM	temp_slk_plots_sum
		GROUP BY temp_slk_plots_sum.query, temp_slk_plots_sum.job,  temp_slk_plots_sum.sign
		ORDER BY sm DESC
	;
