CREATE OR REPLACE FUNCTION return_energy_ranking_job ( p_signature INT ) 
	RETURNS TABLE ( query VARCHAR(50), job BIGINT, sz BIGINT, mean DOUBLE PRECISION, stddev DOUBLE PRECISION ) as $$	
DECLARE 
	q VARCHAR(50);
BEGIN
	...
	CREATE TEMPORARY TABLE temp_slk_final AS
		...
		GROUP BY temp_slk_plots.query, temp_slk_plots.job,  temp_slk_plots.sz
		ORDER BY mean DESC
	;
